...
import { CREDIT_COSTS, CREDIT_PRICE_USD, formatUsd } from "@/entities/pricing"
...
<template>
  <section id="credits" class="section-ground py-[60px] sm:py-[100px]">
    <div class="container relative z-10">
      ...
      <div class="mt-6 flex justify-center" data-aos="fade-up" data-aos-duration="800">
        <span
        ...
        >
          ...
          {{ $t("pricing.costs.rate", { price: formatUsd(CREDIT_PRICE_USD) }) }}
        </span>
      </div>
      ...
      >
        <table class="w-full text-left text-sm">
          <thead class="border-b border-sand-200 bg-sand-50 text-xs font-medium text-sand-500">
            <tr>
              <th scope="col" class="px-4 py-3 sm:px-6">{{ $t("pricing.costs.columns.service") }}</th>
              <th scope="col" class="px-4 py-3 sm:px-6">{{ $t("pricing.costs.columns.credits") }}</th>
          ...
          <tbody class="divide-y divide-sand-200">
            ...
            >
              ...
              <td class="hidden whitespace-nowrap px-6 py-4 tabular-nums text-sand-500 sm:table-cell">
                ≈ {{ formatUsd(cost.credits * CREDIT_PRICE_USD, { cents: true }) }}
              </td>
